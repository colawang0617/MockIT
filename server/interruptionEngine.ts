...
export async function analyzeForInterruption(
    ...
    // Very conservative interruption - let user finish their thoughts naturally
    const wordCount = userText.split(' ').length;
    const hasLongPause = speechDuration > 8000; // 8 seconds of silence (very long pause needed)
    const isTooLong = wordCount > 300; // Allow very long responses - let them speak
    const isVague = hasVagueIndicators(userText);

    // Don't interrupt unless they've spoken a LOT - at least 80 words
    if (wordCount < 80) {
